  ...
  return;
    ...
    try {
     ...
     for (const sp of saleProducts) {
      ...
      try { console.log('saleProducts no momento do envio:', JSON.stringify(saleProducts)); const primeiroProduto = saleProducts[0]; const _p = primeiroProduto?.product as any || products.find(p => p.id === primeiroProduto?.product_id) as any; const nomeProduto = _p ? ((_p.model || '') + ' ' + (_p.color || '')).trim() || 'Produto' : 'Produto'; const blingPayload = { cliente_nome: formData.customer_name || 'Cliente', cliente_cpf: formData.customer_cpf || '', valor_total: totals.totalSalePrice, valor_unitario: primeiroProduto?.unit_price || totals.totalSalePrice, produto: nomeProduto, quantidade: primeiroProduto?.quantity || 1, forma_pagamento: paymentMethods[0]?.method || 'pix', cliente_email: formData.customer_email || '', cliente_telefone: formData.customer_phone || '', endereco_rua: formData.address_street || '', endereco_numero: formData.address_number || '', endereco_bairro: formData.address_neighborhood || '', endereco_cidade: formData.address_city || '', endereco_estado: formData.address_state || 'PR', endereco_cep: formData.address_zip || '', venda_id: String(saleData.id) }; await fetch('https://lwcvrtjykxnipwkhzyem.supabase.co/functions/v1/bling-sync-venda', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(blingPayload) }); } catch (blingErr) { console.error('Bling:', blingErr); } alert('Venda registrada com sucesso!'); console.log('delivery_type:', formData.delivery_type, 'freteSelecionado:', freteSelecionado); if (formData.delivery_type === 'correios' && freteSelecionado) { try { await fetch('https://lwcvrtjykxnipwkhzyem.supabase.co/functions/v1/gerar-etiqueta', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ venda: { ...blingPayload, servico_id: freteSelecionado.id, endereco_rua: formData.address_street, endereco_numero: formData.address_number, endereco_bairro: formData.address_neighborhood, endereco_cidade: formData.address_city, endereco_estado: formData.address_state, endereco_cep: formData.address_zip, endereco_complemento: formData.address_complement } }) }); alert('Etiqueta adicionada ao Melhor Envio!'); } catch(e) { console.error(e); } }
    ...
    try {
      ...
      const payload = {
        ...
        venda_id: String(saleIdToSend),
          forma_pagamento: paymentMethods[0]?.method || 'pix',
      };
